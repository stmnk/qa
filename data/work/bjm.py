...
class Transformer(tf.keras.layers.Layer):  # bjm
...
class TransformerBlock(tf.keras.layers.Layer): # bjm
...
class Attention(tf.keras.layers.Layer): # bjm
...
class Dense3D(tf.keras.layers.Layer): # bjm
...
class Dense2DProjection(tf.keras.layers.Layer): # bjm
...
def get_activation(identifier):  # bjm
...
def gelu(x):  # bjm
  ...
  A smoother version of RELU. Original paper: https://arxiv.org/abs/1606.08415
  Args:
...
def get_initializer(initializer_range=0.02):  # bjm
...
def is_special_none_tensor(tensor): # bjm
  """Checks if a tensor is a special None Tensor."""
  return tensor.shape.ndims == 0 and tensor.dtype == tf.int32


def get_shape_list(tensor, expected_rank=None, name=None): # bjm
...
def assert_rank(tensor, expected_rank, name=None): # bjm
...
def create_attention_mask_from_input_mask(from_tensor, to_mask): # bjm
